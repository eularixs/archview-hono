...
export interface Func {
  ...
  line: number;
  col: number;
  pkg: string; // file path relative to root, no ext (posix)
  generated: boolean;
  exported: boolean;
}
...
export function load(root: string): Result {
  ...
  const isExported = (node: Node): boolean => {
    const d = node as any;
    if (typeof d.isExported === "function") return d.isExported();
    return false;
  };
  const add = (node: Node, recv: string, name: string, sf: SourceFile, exported = false) => {
    if (!name) return;
    ...
    usedIds.add(id);
    const f: Func = { node, id, label: display, recv, name, file: sf.getFilePath(), line, col: column, pkg, generated: isGen(sf), exported: exported || !!recv };
    funcs.set(node, f);
    byDecl.set(node, f);
  };

  for (const sf of sfs) {
    for (const fn of sf.getFunctions()) add(fn, "", fn.getName() ?? "", sf, fn.isExported());
    ...
    for (const v of sf.getVariableDeclarations()) {
      ...
      if (init && (Node.isArrowFunction(init) || Node.isFunctionExpression(init))) {
        ...
        add(init, "", v.getName(), sf, !!stmt && stmt.isExported());
      }
    }
  }
  ...
  const resolveExpr = (expr: Node): Node[] => {
    ...
    // Follow import aliases: a bare imported identifier's symbol is the
    // ImportSpecifier; the real declaration lives behind getAliasedSymbol().
